...
def hillclimber(function, data,t,params0=(0,0,0,0),evalfunc='RMSE',n_iter=1000,stepsize=1,indexdata='full'):
...
#
#     pd.DataFrame({'HC':HC,'SA':SA}).to_csv('./Data/optimal-{}.csv'.format(eval))
#     pd.DataFrame({'HC':HC_conv,'SA':SA_conv}).to_csv('./Data/optimal-{}-conv.csv'.format(eval))
    ...
    for short in shortlist:
        ...
        for s in sizes:
            ...
            if short == 'lessx':
                ...
                index_x = np.where(data[0] > 2)
                index_y = np.where(data[1] > 2)
            else:
                index_x = np.where(data[0] < 2)
                index_y = np.where(data[1] < 2)
            ...
            for i in range(reps):
                #1. Hill climber
                print('REP {}'.format(i))
                RMSE, params, RMSE_list = hillclimber(ODE_error, shortdata, t,n_iter=n_iter,indexdata=indexdata)
                HC_sample.append(RMSE)
            ...
            if short == 'onlypeaks' or short == 'nopeaks':
                break
...
#     pd.DataFrame(RMSE_dict).to_csv('fracdata-{}.csv'.format(eval))
